 ...
 */
window.addEventListener("message",(event=>{window.self!==window.top?"init"===event.data.message&&((zoomObj,userObj,zoomMeetingSdkWebVersion,debugging)=>{const debug=debugging||!1,log=function(){if(debug){const LOG_PREFIX=()=>"**** LYNX **** "+(new Date).toLocaleString()+": ";var args=Array.from(arguments);args.unshift(LOG_PREFIX()),window.console.log.apply(window.console,args)}},error=function(){if(debug){const LOG_PREFIX=()=>"**** LYNX **** "+(new Date).toLocaleString()+": ";var args=Array.from(arguments);args.unshift(LOG_PREFIX()),window.console.error.apply(window.console,args)}},getSessionKeys=()=>{let retval=[];return Object.keys(window.sessionStorage).forEach((function(key){retval.push(key)})),retval},getNewSessionKeys=startArr=>getSessionKeys().filter((x=>!startArr.includes(x)));log("**********************************************");const sessionKeys=getSessionKeys();let language=userObj.lang||"en-US";var inputStr,textarea;-1===language.indexOf("-")&&(language=language.toLowerCase()+"-"+("en"!==language?language.toUpperCase():"US")),window.ZoomMtg.setZoomJSLib("https://source.zoom.us/".concat(zoomMeetingSdkWebVersion,"/lib"),"/av"),window.ZoomMtg.preLoadWasm(),"function"==typeof window.ZoomMtg.prepareWebSDK?window.ZoomMtg.prepareWebSDK():window.ZoomMtg.prepareJssdk(),window.ZoomMtg.i18n.load(language),window.ZoomMtg.i18n.reload(language),document.getElementById("zmmtg-root").style.display="block",log("init meeting with zoom object",zoomObj),log("init meeting with user object",userObj),log("Meeting SDK",window.ZoomMtg.getWebSDKVersion()),log(window.ZoomMtg.checkFeatureRequirements()),window.ZoomMtg.init({leaveUrl:(inputStr=zoomObj.leaveUrl,textarea=document.createElement("textarea"),textarea.innerHTML=inputStr,textarea.value),debug:debug,disableCORP:!window.crossOriginIsolated,enableHD:!0,enableFullHD:!0,isSupportAV:!0,showMeetingHeader:zoomObj.userishost,videoHeader:zoomObj.userishost,isSupportBreakout:!1,isSupportCC:zoomObj.userishost,isSupportChat:zoomObj.userishost,isSupportNonverbal:zoomObj.userishost,isSupportPolling:zoomObj.userishost,isSupportQA:zoomObj.userishost,disableCallOut:!zoomObj.userishost,disableInvite:!zoomObj.userishost,disableRecord:!zoomObj.userishost,disableReport:!zoomObj.userishost,meetingInfo:zoomObj.userishost?["topic","host","participant","mn","dc","pwd","invite","enctype"]:[],success:function(initResp){log("intResponse is ",initResp),window.ZoomMtg.join({signature:zoomObj.signature,sdkKey:zoomObj.sdkKey,meetingNumber:zoomObj.meeting_id,userName:userObj.fullname,userEmail:userObj.email,passWord:zoomObj.password,success:function(joinResp){var _document$getElements;zoomObj.userishost||Array.from(null!==(_document$getElements=document.getElementsByClassName("meeting-info-container--left-side"))&&void 0!==_document$getElements?_document$getElements:[]).forEach((element=>{element.style.display="none"}));const addedKeys=getNewSessionKeys(sessionKeys);addedKeys.length&&window.sessionStorage.setItem("zoomKeys",addedKeys.join(",")),log("joinResp is ",joinResp)},error:function(joinResp){error("joinResp is ",joinResp)}})},error:initResp=>{error("intResponse is ",initResp)}}),log("**********************************************")})(event.data.zoom,event.data.user,event.data.zoomSdkVersion,event.data.debugging):"zoomLeave"===event.data.message&&"redirect"in event.data&&window.location.replace(event.data.redirect)}),!1);
